...
const iipDescription = "IIP-25: Reduce LM for Best Yield and Fund restructured leagues \n https://gov.idle.finance/t/iip-24-m3-2022-leagues-budget-and-idle-incentives-distribution-update/1037";

export default task("iip-25", iipDescription)
.setAction(async (_, hre) => {
  ...
  // using 12 seconds as blocktime (with ETH PoS) we have about 2628000 blocks or about 7200 / day
  // we count 12th Sept as execution time for this IIP, so we have about 2.5 months of
  // IDLE rewards left in the IdleController (end was planned for ~26th November)
  // so we have 1000 / 7200 = 0.13888888888 per block as new rate
  // total spending will be 1000 * 75 = 75000 IDLE to arrive to Nov 26th + 90k IDLE to extend the distribution period to Feb 26th
  // tot spending is 90k + 75k = 165k
  const newIdleControllerRate = toBN('138888888880000000');
  // current distribution is 0.3875 per block so about 2400 IDLE per day (counting old Pow blocktime), so a total of 2400 * 75 = 180000 IDLE
  // the difference is about 180k - 165k = 15k IDLE that we should transfer to the distributor
  // In total we should have 165k for BY and 90k to Gauges (tot 255k). So we are short 
  // by about 90k IDLE that we take from ecosystemFund + ~20k which are already missing
  
  // we can reduce the duration by 20 days, so we would need 75k + 70k = 145k IDLE for BY and 70k for Gauges
  // (+20k missing that will be taken from TL multisig)
  // BUT we only have 180 - 145 = 35k from controller
  // so we should transfer a total of 70 - 35 = 35k from treasury multisig + 20k which are already missing for gauges
  const idleFromController = toBN(35000).mul(ONE);
